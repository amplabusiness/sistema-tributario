...
describe('DocumentParsingAgent', () => {
  ...
  describe('processDocument', () => {
    it('deve processar um documento SPED Fiscal com sucesso', async () => {
      ...
      // Sobrescrever as propriedades do agent
    ...
    it('deve retornar erro se o documento não puder ser processado', async () => {
      ...
      expect(result.validationErrors).toContain('Erro de processamento: Tipo de arquivo não suportado');
      expect(result.processingTime).toBeGreaterThan(0);
    });
  });
  ...
  describe('processBatch', () => {
    it('deve processar múltiplos documentos em lote', async () => {
      ...
      const mockDocumentParser = MockFactory.createDocumentParsingAgentMock();
      mockDocumentParser.documentParser.parseDocument
        .mockResolvedValueOnce(mockResults[0])
        .mockResolvedValueOnce(mockResults[1])
        .mockRejectedValueOnce(new Error('Erro no documento 3'));
      ...
      expect(result.successCount).toBe(2); // Corrigindo para 2 sucessos
      expect(result.errorCount).toBe(1); // Corrigindo para 1 erro
